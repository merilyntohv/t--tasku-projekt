<x-guest-layout>
    ...
    <div class="sm:fixed sm:top-0 sm:right-0 p-6 text-right z-10">
        @auth
            <a href="{{ url('/') }}" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500"></a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Logout</a>
            </form>
        @else
...
@endif
    ...
    <div class="px-4 py-4 bg-[#102E4A]">
        <div>
            <form id="otsing" action="" method="get"
                class="py-4 flex flex-col mt-8 lg:flex  space-y-6 items-center">
                <input type="search" name="search" id="q"
                    class="w-full border-0 p-2 bg-white rounded-full shadow-xl" autocomplete="off"
                    placeholder="Sisesta märksõna">
                <input type="search" name="county" id="q"
                    class="w-full border-0 p-2  bg-white rounded-full shadow-xl" autocomplete="off"
                    placeholder="Maakond">
                <input type="search" name="specialty" id="q"
                    class="w-full border-0 p-2  bg-white rounded-full shadow-xl" autocomplete="off"
            ...
            <div class="container flex flex-col items-center gap-16 mx-auto">
                <div class="grid w-full grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div class="flex flex-col items-center gap-3 px-8 py-10 bg-white rounded-3xl shadow-xl">
                        ...
                        <button
                            class="btn-default overflow-hidden relative w-64 bg-gradient-to-r bg-[#102E4A] text-white py-4 px-4 rounded-xl font-bold uppercase transition-all duration-100 -- hover:shadow-md border border-stone-100 hover:bg-gradient-to-t hover:from-stone-100 before:to-stone-50 hover:-translate-y-[3px]">
